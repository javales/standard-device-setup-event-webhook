#!/bin/bash
# Stanford Device Setup - Log status to webhook endpoint
# Version: 0.1
#
# Description:
...
SDS_REPORT_LOG="/Library/Logs/Stanford/Stanford-DeviceSetup.log"
...
buildSDRDownloadFailurePayload () { # Baseline payload based on the Jamf Setup Manager webhook payload
    ...
    EVENT_DESCRIPTION="*Stanford Device Setup* - resorted to FALLBACK download source"
    EVENT_ERROR_CODE=404
    EVENT_ERROR_MESSAGE="failed to download from PRIMARY download source: https://uit.stanford.edu/sites/default/files/installers/sdr/mac/autoupdater/current/enrollment/SDR%20-%20Enrollment%20Only.dmg)"
